...
export KUBECONFIG=/home/admin/akash-provider/k8s-admin.conf
...
nsdata(){
  microk8s kubectl get ns -l "$md_pfx=true,$md_lid.provider" \
...
nsdata | ldata | while read -r line; do
  ...
  state=$(akash query market lease get \
    --owner "$owner" \
    --dseq  "$dseq" \
    --gseq  "$gseq" \
    --oseq  "$oseq" \
  ...
  if [ "$state" == "closed" ]; then
    echo microk8s kubectl delete ns "$ns" --wait=false
    echo microk8s kubectl delete providerhosts -n lease \
